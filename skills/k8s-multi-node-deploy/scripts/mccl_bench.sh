...
if [[ -z "$WORLDSIZE" ]]; then
    ...
    for gid in "${!group_hostfiles[@]}"; do
        ...
        if [[ -f "$logfile" ]]; then
                ...
                if [[ $num_total_groups -gt 1 ]]; then
                    ...
                    if [[ -n "$oop_slow" ]]; then
                        has_error=1
                        error_msgs+="[Out-of-place busbw significantly lower than best in group]\n$oop_slow"
                    fi
                    if [[ -n "$ip_slow" ]]; then
                        has_error=1
                        error_msgs+="[In-place busbw significantly lower than best in group]\n$ip_slow"
                    fi
                fi
                ...
                if [[ $has_error -eq 1 ]]; then
                    ...
                    error_group_hosts[$gid]="${group_ips[*]}"
                    error_group_details[$gid]="$error_msgs"
                    echo "  [Group $gid] ERROR detected"
                else
                    echo "  [Group $gid] OK"
                fi
        fi
    ...
    if [[ $error_found -eq 1 ]]; then
        echo ""
        echo "ERROR: Some groups failed the MCCL benchmark. See details above."
        echo "Check individual log files in $LOG_DIR for full output."
        exit 1
    else
        echo ""
        echo "All groups passed MCCL benchmark successfully."
    fi
fi
