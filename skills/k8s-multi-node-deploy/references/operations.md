...
**架构**：本地 MCP 客户端 → bastion-k8s-mcp（本地 npx 进程）→ SSH 登录堡垒机 → 在堡垒机上执行 `kubectl`。因此本地直接 `ssh` 到堡垒机常因缺少公钥或 `BatchMode` 而失败，优先复用当前已核验的 MCP；其他已授权连接方式也可使用。配置示例见本 Skill 下的 [`mcp/bastion-k8s.example.json`](../mcp/bastion-k8s.example.json)；Cursor 用户通常粘贴到 `~/.cursor/mcp.json`。
...
**本技能所附 MUSA 模板采用的集群模式（不是通用 K8s 要求）：** `assigment` label + `hostNetwork: true` + `hostPort: 62216` → 必须 `podAntiAffinity`；多机共享数据 **禁止** hostPath。
...
按入口的 Deployment → ReplicaSet → Pod 身份核验流程选择 Ready Pod，核对容器和 PVC 挂载。共享 PVC 只复制一次，容器本地目录逐 Pod 处理。

```bash
kubectl cp -n "$NS" -c "$CONTAINER" ./local "$POD:<verified-container-path>"
```

先核对目标文件与来源，使用明确文件清单；不是整目录覆盖许可。
...
→ kubectl cp 上传脚本/模型
→ 在业务执行授权范围内启动（推理/服务通常到此结束；要 hostfile 则继续 D）
→ Pending 见下文
```
...
使用入口中的 JSON 快照与 `hostfile_from_pods.py`，核对 owner、完整 selector、Ready、唯一节点和资源后生成。不以 Running 状态或 `app=$APP` 标签代替这些检查。
...
# 1. 按入口 JSON 流程生成并审查本次 4 节点 hostfile_4node
...
- Phase 1：两两配对并发测试
- Phase 2：对 Phase 1 失败节点用通过节点交叉验证，提供待复核的可疑 IP
- 输出总结与可疑节点列表；失败组或第二阶段失败均返回非零
...
**适用场景**：已明确授权启动、清理和有限重试的专用 MUSA 训练池。所附脚本是需在目标环境验收的历史运维工具，不构成生产规模或 HA 验收证明。
...
- **守护进程**：`--daemon`、cluster lock 和 PID 文件管理；HA 副本可等待锁，禁止凭心跳超时自动抢占存量锁
...
# 清理残留 GPU 进程
bash stop_all.sh hostfile.128  # 默认只列出候选 PID
```
...
# 准备入口流程核验后的专用池 hostfile_all；整个池的进程清理须在授权内。
...
kubectl exec -it -n $NS $POD -- bash -c "
  cd $MOUNT/<project>
  chmod +x stop_all.sh
  ./stop_all.sh hostfile_all 64  # 预演；执行需 --execute --all-gpu-processes
"
```
...
脚本内部使用 `ssh -o BatchMode=yes -o StrictHostKeyChecking=yes`，因此要求目标节点已配置免密登录（或通过堡垒机中转）。
